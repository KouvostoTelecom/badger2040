...
use uc8151::{Uc8151, UpdateRegion};
...
fn main() -> ! {
    ...
    let region = UpdateRegion::new(0, 48, uc8151::WIDTH, 24).unwrap();

    let tags = ["Dist", "Zokol", "Hasanen", "Kepler"];
    let mut d = 0;
    loop {
        Text::with_alignment(
            tags[d],
        ...
        display.partial_update(region).unwrap();
